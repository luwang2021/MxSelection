from ddselection.data.mx_selection_aov_caviar_v1_training_data_query import *
from ddselection.data.mx_selection_aov_caviar_v1_prediction_data_query import *
from ddselection.data.utils import *
from ddselection.config_aov_v1 import *
...
def load_mxselection_aov_training_data_v1(schema, role, user, password):
    """Load training data for selection aov v1 models
    ...
    nn_dataset = Dataset(
        name='nn',
        query=q_aov_training_data_nn_v1,
    ...
    ac_dataset = Dataset(
        name='ac',
        query=q_aov_training_data_ac_v1,
        date_cols=DATE_SPLIT_COL,
        selected_cols=(
    ...
    da_dataset = Dataset(
        name='da',
        query=q_aov_training_data_da_v1,
        date_cols=DATE_SPLIT_COL,
        selected_cols=(
...
def load_mxselection_aov_prediction_data_v1(
    pred_date,
    schema,
    role,
    user,
    ...
    """Load prediction features for selection aov v1 models
    ...
    nn_pred_features_dataset = Dataset(
        name='nn',
        query=query_nn_pred_features_aov_v1.format(test_date=pred_date),
        date_cols=None,
        selected_cols=(
            NN_NUM_COLS + NN_CAT_COLS + PASSTHROUGH_COLS
        )
    ...
    ac_pred_features_dataset = Dataset(
        name='ac',
        query=query_ac_pred_features_aov_v1.format(test_date=pred_date),
        date_cols=None,
        selected_cols=(
            AC_NUM_COLS + AC_CAT_COLS + PASSTHROUGH_COLS
        )
    ...
    da_pred_features_dataset = Dataset(
        name='da',
        query=query_da_pred_features_aov_v1.format(test_date=pred_date),
        date_cols=None,
        selected_cols=(
            DA_NUM_COLS + DA_CAT_COLS + PASSTHROUGH_COLS
        )
