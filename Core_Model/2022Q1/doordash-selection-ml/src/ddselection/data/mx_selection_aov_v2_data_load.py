...
def load_mxselection_aov_training_data_v2(schema, role, user, password):
    ...
    ac_dataset = Dataset(
        name='ac',
        query=q_aov_training_data_ac_v2,
        date_cols=DATE_SPLIT_COL,
        selected_cols=(
    ...
    da_dataset = Dataset(
        name='da',
        query=q_aov_training_data_da_v2,
        date_cols=DATE_SPLIT_COL,
        selected_cols=(
...
def load_mxselection_aov_prediction_data_v2(
    ...
    ac_pred_features_dataset = Dataset(
        name='ac',
        query=query_ac_pred_features_aov_v2.format(test_date=pred_date),
        date_cols=None,
        selected_cols=(
            AC_NUM_COLS + AC_CAT_COLS + PASSTHROUGH_COLS
        )
    ...
    da_pred_features_dataset = Dataset(
        name='da',
        query=query_da_pred_features_aov_v2.format(test_date=pred_date),
        date_cols=None,
        selected_cols=(
            DA_NUM_COLS + DA_CAT_COLS + PASSTHROUGH_COLS
        )
