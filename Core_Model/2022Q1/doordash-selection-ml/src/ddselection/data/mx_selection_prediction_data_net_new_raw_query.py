from ddselection.data.query_config import NN_TRAIN_5MONTH_TABLE_NAME
...
left join
    mx_SEARCH_COUNT sc
on
    dnn.match_id=sc.store_id
LEFT JOIN
    mx_store_requests sr
ON
    dnn.match_id = sr.match_id
LEFT JOIN
    mx_sp_stats sps
ON
    dnn.starting_point_id=sps.starting_point_id
LEFT JOIN
    mx_submarket_stats sbs
ON
    dnn.submarket_id=sbs.submarket_id
LEFT JOIN
    public.fact_region region
ON dnn.submarket_id = region.submarket_id
