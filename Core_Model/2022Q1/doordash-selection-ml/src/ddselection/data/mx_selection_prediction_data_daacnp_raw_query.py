from ddselection.data.query_config import DANP_TRAIN_5MONTH_TABLE_NAME
...
, mx_BUSINESS_STATS_BEFORE_REACTIVATION AS (
    ...
    --  AND DD_1.ACTIVE_DATE <= MRD_0.activation_date
      AND DD_1.ACTIVE_DATE between dateadd('DAY', -180, MRD_0.activation_date) and MRD_0.activation_date
      AND DD_1.IS_FILTERED_CORE=true
    GROUP BY (
        MRD_0.BUSINESS_ID,
        MRD_0.ACTIVATION_DATE)
...
, mx_SEARCH_COUNT AS (
    ...
    ELSE 'other'
		...
		COALESCE(SBD.TOTAL_SUBTOTAL_BEFORE/NULLIF(SBD.NUM_DAYS_ACTIVE_BEFORE, 0), 0) AS TOTAL_SUBTOTAL_PER_ACTIVE_DATE_BEFORE,
		COALESCE(SBD.NUM_ORDERS_BEFORE::FLOAT/NULLIF(SBD.NUM_DAYS_ACTIVE_BEFORE, 0), 0) AS NUM_ORDERS_PER_ACTIVE_DATE_BEFORE,
    ...
    LEFT OUTER JOIN mx_STORE_STATS_BEFORE_DEACTIVATION AS SBD
               ON S_0.NIMDA_STORE_ID = SBD.NIMDA_STORE_ID AND S_0.MOST_RECENT_DEACTIVATION_TIMESTAMP = SBD.MOST_RECENT_DEACTIVATION_TIMESTAMP
    LEFT OUTER JOIN mx_BUSINESS_STATS_BEFORE_REACTIVATION AS BSD
               ON S_0.BUSINESS_ID = BSD.BUSINESS_ID AND S_0.activation_date = BSD.activation_date
    LEFT OUTER JOIN mx_SP_STATS_COMBINED AS SPC
               ON SPC.NIMDA_STORE_ID = S_0.NIMDA_STORE_ID AND SPC.STARTING_POINT_ID = S_0.STARTING_POINT_ID AND S_0.activation_date = SPC.activation_date
    LEFT OUTER JOIN geo_intelligence.public.maindb_starting_point AS SP
               ON SP.ID = S_0.STARTING_POINT_ID
    LEFT OUTER JOIN mx_store_requests AS sr
               ON s_0.match_id=sr.match_id
    LEFT OUTER JOIN PUBLIC.fact_selection_intel_mx_raw AS RX
               ON rx.VENDOR_1_ID = S_0.VENDOR_1_ID
    LEFT OUTER JOIN
        public.fact_region region
    ON SP.submarket_id = region.submarket_id
    WHERE 1 <> 0
